...
class PhotoCatalogApp(tk.Tk):
    ...
    def _build_examples_tab(self):
        ...
        self._nb.add(outer, text="  Examples  ")
        ...
        tab.after(50, lambda: canvas.configure(
        ...
        ttk.Label(
            tab,
            text="Click Load on any example to pre-fill the form, then switch tabs and press ▶.",
            foreground="gray",
        ).grid(row=0, column=0, sticky="w", pady=(0, 8))

        examples = [
            (
                "Ingest Mac Photos — default library",
                "Build Catalog",
                "Most common starting point. Leave source path blank and macOS\n"
                "finds the default Photos library automatically.",
                lambda: self._load_ingest("MacPhotos", "", False),
            ),
            (
                "Ingest Mac Photos — specific library",
                "Build Catalog",
                "Use when you have multiple Photos libraries. After loading,\n"
                "click Browse… next to Source path to select the .photoslibrary bundle.",
                lambda: self._load_ingest("MacPhotos", "", False),
            ),
            (
                "Ingest Lightroom Classic catalog",
                "Build Catalog",
                "Reads directly from the Lightroom SQLite file (.lrcat).\n"
                "After loading, click Browse… to select your .lrcat file.",
                lambda: self._load_ingest("Lightroom", "", False),
            ),
            (
                "Ingest a folder on disk",
                "Build Catalog",
                "Scans a folder tree with exiftool to extract metadata from every image.\n"
                "After loading, click Browse… to select the root folder to scan.",
                lambda: self._load_ingest("Folder", "", False),
            ),
            (
                "Fresh ingest — replace all existing data",
                "Build Catalog",
                "Drops and recreates the database before ingesting. Use when you\n"
                "want to start over rather than add to an existing catalog.",
                lambda: self._load_ingest("MacPhotos", "", True),
            ),
            (
                "Find duplicates — unified (recommended)",
                "Find Duplicates",
                "Combines perceptual hash + metadata + filename via union-find.\n"
                "Min score 2: keeps groups matching on at least two signals.\n"
                "Group #1 is always the strongest match.",
                lambda: self._load_find("unified", 2),
            ),
            (
                "Find duplicates — strict  (score ≥ 4)",
                "Find Duplicates",
                "Raises minimum score to 4 — only high-confidence matches kept.\n"
                "A score of 4 means 4 of: phash, dims, date, type, size, name agree.\n"
                "Produces fewer groups but each is very likely a true duplicate.",
                lambda: self._load_find("unified", 4),
            ),
            (
                "Find duplicates — perceptual hash only",
                "Find Duplicates",
                "Matches photos with an identical perceptual hash (pixel similarity).\n"
                "Fast and precise — finds exact or near-exact pixel copies.\n"
                "Misses duplicates with different crops or re-exported files.",
                lambda: self._load_find("phash", 2),
            ),
            (
                "Find duplicates — all strategies",
                "Find Duplicates",
                "Runs unified first, then each individual strategy in sequence.\n"
                "Useful for comparing strategies. Unified is recommended for\n"
                "actual deduplication work.",
                lambda: self._load_find("all", 2),
            ),
            (
                "Launch the duplicate browser",
                "Browse Duplicates",
                "Opens a local web server at http://127.0.0.1:8765 with a two-pane viewer.\n"
                "Left: duplicate groups with signal dots. Right: photo cards with thumbnails,\n"
                "metadata diffs highlighted in red, and Keep / Discard buttons.",
                lambda: self._load_browse("8765"),
            ),
        ]

        for i, (title, badge, desc, loader) in enumerate(examples):
            card = ttk.Frame(tab, relief="groove", padding=8)
            card.grid(row=i + 1, column=0, sticky="ew", pady=(0, 6))
            card.columnconfigure(1, weight=1)

            ttk.Button(card, text="Load", command=loader, width=6).grid(
                row=0, column=0, rowspan=2, padx=(0, 10), sticky="n"
            )
            title_row = ttk.Frame(card)
            title_row.grid(row=0, column=1, sticky="ew")
            ttk.Label(title_row, text=title, font=("", 12, "bold")).pack(side="left")
            ttk.Label(
                title_row, text=f"  →  {badge}", foreground="#007aff", font=("", 11),
            ).pack(side="left", padx=(6, 0))
            ttk.Label(card, text=desc, foreground="gray", justify="left").grid(
                row=1, column=1, sticky="w"
            )

    def _load_ingest(self, source: str, src_path: str, fresh: bool):
        self._source.set(source)
        self._src_path.set(src_path)
        self._fresh.set(fresh)
        self._refresh_ingest()
        self._nb.select(0)

    def _load_find(self, strategy: str, min_score: int):
        self._strategy.set(strategy)
        self._min_score.set(min_score)
        self._nb.select(1)

    def _load_browse(self, port: str):
        self._port.set(port)
        self._nb.select(2)
